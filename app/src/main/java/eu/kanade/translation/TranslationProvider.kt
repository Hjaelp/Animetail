...
class TranslationProvider(
    ...
    /**
     * Returns the translations directory for a manga. For internal use only.
     *
    ...
    /**
     * Returns the transltion directory for a source if it exists.
     *
     * @param source the source to query.
     */
    fun findSourceDir(source: MangaSource): UniFile? {
        return translationsDir?.findFile(getSourceDirName(source))
    }
     ...
     * @param source the source of the chapter.
     */
    fun findChapterTranslations(chapters: List<Chapter>, manga: Manga, source: MangaSource): Pair<UniFile?, List<UniFile>> {
        val mangaDir = findMangaDir(manga.title, source) ?: return null to emptyList()
        return mangaDir to chapters.mapNotNull { chapter ->
            mangaDir.findFile(getValidChapterName(chapter.name, chapter.scanlator))
        }
    }
     ...
     * @param chapterScanlator scanlator of the chapter to query
     */
    fun getChapterName(chapterName: String, chapterScanlator: String?): String {
     ...
     * @param chapterScanlator scanlator of the chapter to query
     */
    fun getValidChapterName(chapterName: String, chapterScanlator: String?): String {
        val chapterFileName = getChapterName(chapterName, chapterScanlator)
        return "$chapterFileName.json"
    }
}
